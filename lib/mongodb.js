...
class mongoDB {
    ...
    getRequestType(params) {
        if (params.type == 'customer') {
            return config.enums.type.CUSTOMER;
        } else if (params.type == 'user') {
            return config.enums.type.USER;
        } else if (params.type == 'visitor') {
            return config.enums.type.VISITOR;
        } else if (params.type == 'conversation') {
            return config.enums.type.CONVERSATION;
        } else {
            return 0;
        }
    }
    ...
    async insert(params) {
        ...
        return new Promise(async (resolve, reject) => { 
            ...
            try {
                if (type == config.enums.type.CUSTOMER) { //create DB
