            ...
            try {
                for (const replica in replicas) {
                    if (replica === 'primary') { //ignore primary instance
                        ...
                        that.logger.debug(`getSecondaryHandler: Connecting to DocumentDB at secondary replica ${replicas[replica]}`);
                        ...
                        try {
                            ...
                            that.logger.info(`getSecondaryHandler: Connected to DocumentDB at secondary replica ${replicas[replica]}`);
                        } catch (err) {
                            that.logger.error(`getSecondaryHandler: The connection to DocumentDB at secondary replica ${replicas[replica]} failed. Error: ${err}`)
                            that.secondaryHandler[replica] = null;
                        }
                    } else {
                        that.logger.error(`getSecondaryHandler: replica details empty for ${replica}`);
                    }
                }
                resolve(that.secondaryHandler);
            } catch (err) {
                that.logger.error(`getSecondaryHandler: Error on getting secondaray connections. Error ${err}`);
                reject(err);
            }
        })
    }
    ...
    async collectReplicas() {
        ...
        return new Promise(async (resolve, reject) => { 
            try {
                ...
                try {
                    await that.getSecondaryHandler(); //create a connection handlers for secondary connection
                } catch (err) {
