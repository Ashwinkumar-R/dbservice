...
/*      - Find out the cluster and make connection to replicas
/*      - Execute the queries and send back result to server
...
class mongoDB {
    ...
    ************************************/
    ...
    async getFindHandler(params) {
        return new Promise(async (resolve, reject) => {
            if (params.operation == 'aggregate') { //treat aggregate as special case
                this.secondaryHandler.replica3 ? resolve(this.secondaryHandler.replica3) : reject(`empty db handler, coult not query`);
            ...
            try {
                if (that.isConnected()) {
                    if (type == that.enums.type.CUSTOMER) { //no find on DB details
                        ...
                        var [err,res] = [null, null];

                        if (params.operation && params.operation == 'aggregate') { //aggregate
                            [err,res] = await to(db.collection(params.type).aggregate(JSON.parse(query)).toArray());
                        } else if (document == 'fineOne') { //findOne
                            [err,res] = await to(db.collection(params.type).findOne(JSON.parse(query))); 
                        } else { //find all
                            [err,res] = await to(db.collection(params.type).find(JSON.parse(query)).toArray());
                        }
