            ...
            try {
                if (that.isConnected()) {
                    ...
                    if (type == that.enums.type.CUSTOMER) { //delete DB
                        ...
                        let query = (type == that.enums.type.CONVERSATION) ? {"name":params.value} : {"email":params.value}; //record to be deleted 
                        
                        let [err,res] = await to(db.collection(params.type).deleteOne(query));

                        if (err) {
                            let msg = `Error deleting record ${params.value} from DB:${params.customer}, collection:${params.type}, Error: ${err}`;
                            that.logger.debug(`delete: ${msg}`);
                            reject(msg);
                        } else {
                            if (res.result.n != 0 && res.result.ok == 1) {
                                ...
                                reject (`record ${params.value} from DB:${params.customer}, collection:${params.type} not found for deleting`);
                            }
                        }                    
                    }
                } else {
